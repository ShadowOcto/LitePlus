...
public class Stealer extends Module {
    ...
    @Name("FirstItemDelay")
    @Clamp(min = 0, max = 500)
    public static float firstItemDelay = 10f;
    ...
    public void onPacketSend(PacketEvent e) {
        if (blink && e.getPacket() instanceof C03PacketPlayer) {
    ...
    public void onUpdate(PlayerUpdateEvent event) {
        if (!GuiChest.firstItem.isDelayComplete(firstItemDelay))
            return;

        if (!isChest && onlychest)
            return;
        ...
        if (mc.thePlayer.openContainer != null) {
            if (mc.thePlayer.openContainer instanceof ContainerChest) {
                ...
                if (isChestEmpty(c) && openGuiHelper.isDelayComplete(800) && time.isDelayComplete(100)) {
                    mc.thePlayer.closeScreen();
                }

                for (int i = 0; i < c.getLowerChestInventory().getSizeInventory(); ++i) {
                    if (c.getLowerChestInventory().getStackInSlot(i) != null) {
                        if (time.isDelayComplete(nextDelay) && (itemIsUseful(c, i) || trash)) {
                            nextDelay = (int) (delay);
    ...
    private boolean itemIsUseful(ContainerChest c, int i) {
        ...
        if ((item instanceof ItemAxe || item instanceof ItemPickaxe) && tools) {
            return true;
        }

        if (item instanceof ItemFood)
            return true;
        if ((item instanceof ItemBow || item == Items.arrow) && bow)
            return true;
